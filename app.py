...
# Resilience fix: some earlier data-file versions may not yet include the
# final "structural_ownership_barrier_score" column, only its three raw
# components. Compute it here if missing, so the app works regardless of
# which vintage of structural_ownership_barrier.csv is present in the repo.
if "structural_ownership_barrier_score" not in barrier.columns:
    required_raw_cols = ["employment_ownership_gap", "ownership_vs_ssa_avg", "size_trend_small_to_large"]
    if all(c in barrier.columns for c in required_raw_cols):
        def _z(s, flip=1):
            return flip * (s - s.mean()) / s.std()
        barrier = barrier.copy()
        barrier["structural_ownership_barrier_score"] = (
            _z(barrier["employment_ownership_gap"]) +
            _z(barrier["ownership_vs_ssa_avg"], -1) +
            _z(barrier["size_trend_small_to_large"], -1)
        ) / 3
    else:
        # Last-resort fallback: neutral score, with a visible warning rather
        # than a silent wrong number or a crashed app.
        st.warning(
            "structural_ownership_barrier.csv is missing expected columns -- "
            "the Ownership Access component of the Social pillar is showing a "
            "neutral placeholder (0) until this data file is corrected."
        )
        barrier = barrier.copy()
        barrier["structural_ownership_barrier_score"] = 0.0

# Avoid a merge-suffix collision: if full_country_comparison.csv already carries
# its own structural_ownership_barrier_score column (e.g. an older data vintage),
# pandas would rename both copies to *_x/*_y instead of keeping the plain name,
# which is what was causing the KeyError below. Drop it first so the barrier
# file's version is always the one that survives.
if "structural_ownership_barrier_score" in full.columns:
    full = full.drop(columns=["structural_ownership_barrier_score"])

df = full.merge(
    barrier[["country", "structural_ownership_barrier_score"]],
    on="country",
    how="left",
)

if df["structural_ownership_barrier_score"].isna().any():
    missing = df.loc[df["structural_ownership_barrier_score"].isna(), "country"].tolist()
    st.warning(
        "No structural ownership barrier data found for: "
        f"{', '.join(missing)}. Filling with a neutral placeholder (0) so the app doesn't crash."
    )
    df["structural_ownership_barrier_score"] = df["structural_ownership_barrier_score"].fillna(0.0)
...
with tab0:
    ...
    st.plotly_chart(fig0b, width='stretch')
...
with tab3:
    ...
    st.dataframe(hist[["country", "year_early", "ownership_early", "year_recent", "ownership_recent",
                        "ownership_change_per_year"]].round(2), width='stretch', hide_index=True)
    st.caption(
